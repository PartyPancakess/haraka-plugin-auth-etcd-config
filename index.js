...
function load_users_config(domains) {
  ...
  for (var key in domains) {
    if(key.substr(key.length-5) === "/user") {
...
exports.load_config = function () {
  ...
  .then(domains => {
    if (domains) {
      ...
      for (domain in domains) {
        if (domain.substr(domain.length-5) === "/user") {          
  ...
  .then(watcher => {
    watcher
      ...
      .on('put', res => {
        if (res.key.toString().substr(res.key.toString().length-5) === "/user") {          
...
exports.domain_check = function (next, connection, params) {
  if (connection.relaying) {    
    ...
    var domain_config = "config/mta/domains/" + domain + "/user";
